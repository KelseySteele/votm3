(function () {
    ...
    angular.module('votm').controller('faqCtrl', function () {
        var self = this;
        //data
        self.faqs = questions;
        ...
        //tabs
        self.tab = 'general';  //initialize the general tab open
        self.faqcategory = 'general'; //initialize the filter category
        self.faqtemplate = 'views/faqtemplate.html'; //this is the template for each panel
        ...
        self.selectTab = function (setTab) {
            self.tab = setTab;
            self.faqcategory = setTab;  //used to set the filter for the categories
        };
    ...
    var categories = ['general', '3g&4g', 'equipmentoptions', 'usingequipment', 'kencastservices', 'miscellaneous'];
    ...
    var questions = [
...

            {
                ...
                'Vazzt Capture Software will automatically read the default video resolutions available from the camera you attach to the laptop/computer with the Vazzt Capture Server software.  It will present these options to you.  Typical options include the following and can be used in these typical setups:</p>' +
                        ...
                        '</ul>' +
                '<p>However, at this upper resolution you can also use multiple new carrier IP Networks (LTE and/or WiMAX) to get enough bandwidth to support this quality resolution Ð in the range of 2.5-4.5 Mbps).  Currently you may need a server-class computer and a broadband wire connection to the Internet to use the Vazzt Capture Server software at this resolution, until multiple 4G networks are available to be bonded together.</p>'
            },
            {
                category: 'general',
                number: '5',
